...
[`polaris#4522`](https://github.com/apache/polaris/issues/4522) proposes that Polaris — historically Layer A — also host Layer B objects. This is good: Polaris already has the namespace, identity, and grant primitives Layer B needs to ride on. The interesting question is whether to host Layer B as *just YAML* (vanilla OSI) or as *full Navigator bundles* (OSI + Croissant + CDIF + DID + ODRL). The argument for the bundle form is in [`semantic-layer-report.md` §5](../semantic-layer-report.md#5-proposal-querygraph-ai-navigator-inside-apache-polaris-aligned-with-osi-and-ods).
